...
from datacode import ELD_dataset
...
def test(model, args, loader_test, device):
    ...
    for batch, (lr, hr,x) in enumerate(tqdm(testLoader, ncols=80)):
        ...
        vutils.save_image(image3, '/data/zhengchuanjun/UTVNet/result/{}/{}.png'.format(args.data_name,''.join(x)))
...
def main():
    ...
    if args.data_name=='sRGBSID':
     ...
     test_gt_dir = './dataset/sRGBSID/gt/test/'.format(args.data_name)
     loaderTest = ELD_dataset.rgbDataset(test_input_dir, test_input_dir2, test_gt_dir, 'test', '512', args.data_name)

    else:
     test_input_dir = './dataset/ELD/{}/'.format(args.data_name)
     test_input_dir2 = ''
     test_gt_dir = './dataset/ELD/{}g/'.format(args.data_name)
     loaderTest = ELD_dataset.rgbDataset(test_input_dir, test_input_dir2, test_gt_dir, 'test', '1024', args.data_name)
